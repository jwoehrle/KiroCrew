...
/** Remove clone credentials before a repository reaches consent UI or echo. */
export function credentialFreeRepository(raw?: string): string | undefined {
  ...
  if (schemeAt >= 0) {
    ...
    if (userinfoAt >= 0) {
      return raw.slice(0, authorityAt) + authority.slice(userinfoAt + 1) + raw.slice(suffixAt)
    }
    return raw
  }
  // Common scp form: user@host:path. Bare paths containing '@' stay unchanged.
...
export function useTrustGate(retryEnable: (name: string) => Promise<void>) {
  ...
  const open = useCallback((app: TrustAppTarget, retry?: (name: string) => Promise<void>) => {
    setGate({
      app: { ...app, trustRepository: credentialFreeRepository(app.trustRepository) },
